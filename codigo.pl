frase(Salida)-->gn(Suj,Persona,Numero),% sujeto de la frase
				pa(A,PersonaA,Numero),% pronombre atono de la frase
				...
				verbo_aba(E),% raiz del verbo acab
				resto2(R),% el resto de palabras despues del verbo
				{componer1(Salida,Suj,A,V,N,E,R)}.
				
que()-->[que].% el siguiente elemento de la lista es "que"
si()-->[si].% el siguiente elemento de la lista es un "si"
si()-->[].% para que si la frase no tiene un si ,es decir que no es una pregunta la pueda analizar tambien


% el resto de palabras que estan en la frase despues del verbo
% cambia llos pronosmres personales que corresponde
resto2([])-->[].
resto2([X|Xs])-->[P],
				{pro_personal(X,P)},
				resto(Xs).
...
% debuelve la raiz del verbo que acababa en aba
verbo_aba(R)-->[V],
				{name(V,Vs),
...
% cambia los verbos que acaban en me por acabados por te

cambio_te([],[]).
cambio_te([P|Aux],[X|Xs]):-
						name(te,Ts),
						name(X,Vs),
						append(Y,Ts,Vs),
						name(me,Ms),
...
cambio_te([X|Aux],[X|Xs]):-cambio_me(Aux,Xs).


% compone la salida en forma de frase en estilo directo
...
componer1(Salida,S,A,V,N,era,R):-
							append(S,A,Aux1),
							append(Aux1,V,Aux2),
							append(Aux2,N,Aux3),
							append(Aux3,[:],Aux4),
...
% el grupo nominal puede ser un nombre proio solo o un articulo y un combre propio				   
gn([N], 3, Numero) --> [N],
                           {es_nombre(N, Numero) }.
...
% primer verbo dijo o pregunto
primer_verbo([V], Persona, Numero) -->
                            [V],
                            { es_verbo(V, Persona, Numero)}.

%resto de oracion cambiando los pronombres personaes qe corresponda							
resto([])-->[].
resto([X|Xs])-->[P],
				{pro_personal(P,X)},
				resto(Xs).
...
% cambia los te que estan al final de los verbos por me	
cambio_me([],[]).
